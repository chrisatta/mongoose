...
-spec start_listener(mongoose_listener:options()) -> ok.
...
-spec start_link(mongoose_listener:options()) -> any().
start_link(Opts) ->
    supervisor:start_link(?MODULE, Opts).

-spec init(mongoose_listener:options()) ->
...
-spec start_accept_loop(Socket :: port(),
                        Module :: module(),
                        Opts :: mongoose_listener:options()) -> {ok, pid()}.
...
-spec accept_loop(Socket :: port(),
                  Module :: module(),
                  Opts :: mongoose_listener:options()) -> no_return().
...
-spec make_childspec(Id :: term(), ListenSock :: port(),
                     Module :: module(), Opts :: mongoose_listener:options()) ->
...
-spec listen_tcp(mongoose_listener:options()) -> port().
